...
*** Option niveau des diapositives :
...
*** présenter la table des matières à chaque section
...
Plusieurs blocs sur une même diapositive :
voir formation
...
inkscape va être appelé par (xe)latex pour transformer
le SVG en PDF. Dans ce cas, il est nécessaire d'invoquer
(xe)latex avec l'option --shell-escape :
